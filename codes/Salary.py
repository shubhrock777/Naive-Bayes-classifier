...
train_df = pd.read_csv("D:/BLR10AM/Assi/14.naive bayes/Datasets_Naive Bayes/SalaryData_Train.csv")

test_df = pd.read_csv("D:/BLR10AM/Assi/14.naive bayes/Datasets_Naive Bayes/SalaryData_Test.csv")
...
#######feature of the dataset to create a data dictionary
description  = ["age of person ,important",
                "Job type ,important",
                "qualification ,important",
                 "qualification in number same as education column",
                ...
                "loss",
                "expenses,important",
                "born in a particular place · belonging to a person since birth or childhood",
                "Salary of person ,important"]

d_types =["ratio","nominal","oridinal","count","nominal","nominal","nominal","nominal","binary","ratio","ratio","ratio","nominal","binary"]
...
#checking unique value in salary column
train_df['Salary'].unique()

import numpy as np

# converting ' >50K' to 0 and <=50K to 1 for salary columns 
df['Salary'] = np.where(df['Salary'] == ' <=50K', '1', df['Salary'])
df['Salary'] = np.where(df['Salary'] == ' >50K', '0', df['Salary'])

"""
for ele in train_df["Salary"]:
    if ele==' <=50K':
        train_df["Salary"]=1
    else ele==' >50K' :
        train_df["Salary"]=0 """
...
#checking unique value in capitalgain, column with count
df['capitalgain'].unique()
capitalgain_uni_count = pd.value_counts(df['capitalgain'])
capitalgain_uni_count

df['capitalloss'].unique()
capitalloss_uni_count = pd.value_counts(df['capitalloss'])
capitalloss_uni_count


df['hoursperweek'].unique()
hoursperweek_uni_count = pd.value_counts(df['hoursperweek'])
hoursperweek_uni_count

df['native'].unique()
native_uni_count = pd.value_counts(df['native'])
native_uni_count
...
boxplot = df.boxplot(column=["age","capitalgain","capitalloss","hoursperweek"])

#creatind dataframe with only with (discrete,continuous ,output)
X_c=df.iloc[:,[0,11]]
X_d=df.iloc[:,[1,2,4,5,6,7,8,12]]
Y=df.iloc[:,[13]]
